<script lang="ts">
	...
	const link = 'border-l-rail text-rail-ink font-medium hover:bg-rail-active';
...
<aside
	...
	class="bg-rail border-rail-line flex min-w-0 flex-col border-b lg:sticky lg:top-0 lg:h-screen lg:w-65 lg:shrink-0 lg:border-r lg:border-b-0"
>
	...
	>
		{#each groups as group (group.id)}
			{#if group.label}
				...
				<p
					id={group.id}
					class="text-eyebrow text-rail-ink-2 border-rail-line mx-0.5 flex-none border-l py-1 pl-3.5 uppercase lg:mx-0 lg:mt-4 lg:mb-1.5 lg:border-l-0 lg:py-0"
				>
					{group.label}
				</p>
			{/if}
					...
					<li>
						...
						{:else}
							...
							<span aria-disabled="true" class={`${row} ${soon}`}>
								...
								<span class="min-w-0">{item.label}</span>
								<span
									class="text-eyebrow text-rail-ink-2 bg-rail-active ml-auto flex-none rounded-full px-1.5 py-0.5 font-mono uppercase"
								>
	...
	<div
		class="border-rail-line mt-auto flex flex-none flex-row flex-wrap items-center gap-3 border-t px-4 py-4 lg:flex-col lg:items-stretch"
	>
		...
		<form method="POST" action="/logout" class="ml-auto lg:ml-0">
			<button
				type="submit"
				class="border-rail-line text-rail-ink hover:bg-rail-active rounded-control text-caption flex items-center justify-center gap-2 border px-3 py-2 font-medium lg:w-full"
			>
				<svg
